...
class GameSetupFactoryImpl : GameSetupFactory {
    ...
    override fun handleMoveCard(
        ...
        return when (currentGameState.state) {
            ...
            CardState.HINTED -> handleEmptySlotClicked(currentGameState)
            CardState.SELECTED,
            CardState.SUCCESS,
    ...
    private fun resetStockToDeck(
        ...
        val newDealCount = if (currentDealCount > 1) currentDealCount - 1
        else return StockClickResult(currentStacks, 0)

        val resetCards = allCardsFromStockPlay.map { it.copy(isFaceUp = false) }
        val newStock =
            WellCardStack(cards = resetCards, address = WellSlotAddress(WellSlotType.STOCK))
